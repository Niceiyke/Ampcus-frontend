...
interface LocalGovernmentSelectorProps {
    ...
    onSelectLocalGovernment: (lga: string) => void;
}

interface LocalGovernmentsData {
    [key: string]: string[];
}


const LocalGovernments: LocalGovernmentsData = {
    'Abia': ['Aba North', 'Aba South', 'Arochukwu', 'Bende', 'Isiala Ngwa North', 'Isiala Ngwa South', 'Ikwuano', 'Isuikwuato', 'Obi Ngwa', 'Ohafia', 'Osisioma', 'Ugwunagbo', 'Ukwa East', 'Ukwa West', 'Umuahia North', 'Umuahia South', 'Umu Nneochi'],
...
const LocalGovernmentSelector: React.FC<LocalGovernmentSelectorProps> = ({ selectedState, onSelectLocalGovernment }) => {
    const [localGovernments, setLocalGovernments] = useState<string[]>([]);

    const handleLocalGovernmentChange = (e: ChangeEvent<HTMLSelectElement>) => {
        const selectedLGA = e.target.value;
        onSelectLocalGovernment(selectedLGA);
    };

    useEffect(() => {
        if (selectedState && LocalGovernments[selectedState]) {
            setLocalGovernments(LocalGovernments[selectedState]);
        } else {
            setLocalGovernments([]);
        }
    }, [selectedState]);
    ...
    return (
        <div>
            <label htmlFor="localGovernment">Select Local Government:</label>
            <select id="localGovernment" onChange={handleLocalGovernmentChange}>
